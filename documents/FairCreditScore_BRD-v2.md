...
> **A note on UPI data access.** NPCI does not — and by design will not — publish a third-party SDK or API for fetching a user's UPI transaction history. UPI is a payment-execution rail; transaction history lives inside each user's bank. FairCreditScore therefore acquires UPI behavioural data the same way every regulated alternative-data lender in India does: through the **RBI Account Aggregator (AA) framework**, where a user's consented bank statement contains every UPI inflow and outflow with VPA, counterparty, amount, and narration — digitally signed and tamper-proof.

The platform ingests four primary categories of data, in order of priority:

**1. Bank statement data via the Account Aggregator framework (primary rail).**
Consent-based, structured, RBI-regulated data flow that delivers a user's full bank transaction history — including all UPI debits and credits — in seconds. We register as a Financial Information User (FIU) via an anchor NBFC partner and integrate with multiple AA licensees (e.g. Finvu, OneMoney, Setu, NeSL) for redundancy. AA currently covers ~38% of Indian borrowers and is expanding rapidly.

**2. Bank statement PDF analysis (fallback for non-AA banks).**
For the ~62% of borrowers whose banks are not yet AA-enabled (cooperative banks, regional rural banks, several small finance banks), we fall back to user-uploaded PDF bank statements parsed via partner OCR engines (Perfios, FinBox, AZAPI.ai, or equivalent). Same UPI data; less elegant ingestion path; full coverage.

**3. OAuth-based direct platform integrations.**
With explicit user consent, we connect directly to platforms that publish OAuth-based data APIs:

- **Online purchase history** — Amazon, Flipkart, Zomato, Swiggy, BigBasket order histories.
- **Digital cashbook data** — Khatabook, OkCredit, and equivalent SMB-ledger apps for small shopkeepers and vendors.
- **Gig-platform earnings** — Swiggy, Zomato, Uber, Ola driver-partner dashboards (where partner agreements permit).
- **Employee payroll records** — particularly from blue-collar and gig employers via direct B2B partnerships.

**4. Future: Credit Line on UPI (CLOU) behavioural data (Phase 3+).**
Once we have established lender partnerships, CLOU products generate granular UPI-drawdown behaviour visible to the lender. We will partner with lenders running CLOU products to receive consented post-disbursal behavioural feeds, sharpening repayment-stage models.

**5. Future: Utility, telecom, and rent payment history (v2).**
Electricity, water, gas, mobile recharges, and rent — sourced via specialised partners (e.g. CredAvenue, NoBroker for rent, telco APIs).

> **What we explicitly do NOT do:** We do not become a TPAP, we do not claim direct NPCI/UPI API access, we do not scrape device data (contacts, SMS, location), and we do not buy "UPI APIs as a service" from grey-market resellers (NPCI explicitly forbade this in October 2024). Our entire data stack is consent-based, regulator-aligned, and DPDP Act 2023 compliant.
...
- **Account Aggregator integration as the primary data rail**, with two AA partners (e.g. Finvu, OneMoney) for redundancy.
- **PDF bank statement parsing fallback** (via Perfios / FinBox / AZAPI.ai) for borrowers on non-AA-enabled banks.
- OAuth integration with at least one digital cashbook partner (Khatabook or OkCredit) and one e-commerce platform.
...
- iOS app launch.
- Expanded AA coverage as more cooperative banks, RRBs, and SFBs come online.
- Broader e-commerce integrations (Amazon, Flipkart, Zomato, Swiggy, BigBasket, Myntra) for richer spending signals.
- Additional cashbook and gig-platform partnerships beyond Phase 1 anchors.
...
- Champion-challenger pilots versus CIBIL on shared applicant pools.
...
- **CLOU lender partnerships** — consume consented post-disbursal UPI behavioural feeds from partner lenders running Credit Line on UPI products to enhance repayment-stage models.
...
- **Becoming a TPAP (Third-Party Application Provider) on UPI** — this is a payments-business path, not a scoring-business path. UPI behaviour is acquired via AA, not by operating a payment app.
- **Direct NPCI/UPI API integration for transaction history** — NPCI does not publish such an API and has explicitly forbidden non-payment use of UPI APIs (October 2024 NPCI circular).
...
| Data ingestion | AA FIU SDK (Finvu / OneMoney / Setu); bank statement parsing (Perfios / FinBox / AZAPI.ai); OAuth integrations for e-commerce, cashbook, and gig platforms |
...
- Account Aggregator licensee partnerships (Finvu, OneMoney, NeSL, Setu, etc.) — primary rail for consented bank-statement and UPI behavioural data.
- Bank statement analyser partnership (Perfios, FinBox, or AZAPI.ai) — fallback rail for borrowers on non-AA-enabled banks.
- OAuth-based platform partnerships (Khatabook / OkCredit for cashbook; Amazon / Flipkart / Zomato / Swiggy for e-commerce; Swiggy / Zomato / Uber / Ola for gig-platform earnings).
...
| NPCI | National Payments Corporation of India — operator of UPI and other retail payment rails; an initiative of RBI and IBA. |
| NTC | New-to-Credit — borrower with no prior credit history. |
| PSP | Payment Service Provider — RBI-licensed bank that connects to UPI and processes transactions. |
...
| TPAP | Third-Party Application Provider — consumer UPI app (e.g. PhonePe, Google Pay, Paytm) operating via a sponsoring PSP bank. FairCreditScore is **not** a TPAP. |
...
- NPCI Circular (Oct 2024): Warning against unauthorised use of UPI APIs for non-payment purposes.
- NPCI OC-215/2025-26: UPI API Security Guidelines mandating rate limits, audit, and customer-vs-system call differentiation (effective Aug 2025).
